...
def create_linear_stack(N:int, first_dim:int, hidden_dim:int, dropout:float=0.0) -> nn.Sequential:
    '''
    ...
    dropout: If > 0, adds a dropout layer with this probability value after each ReLU layer in the stack
    '''
    ...
    for i in range(N):
        ...
        if dropout > 0:
            linear_stack.append(nn.Dropout(p=dropout))      # optional dropout layer after each shared layer
    return linear_stack
...
class BaseHomogenousModel(torch.nn.Module):
    def __init__(self, num_hops:int,
        ...
        # GNN layers
        # ---------------------------
        self.gat_layers.append(GATConv(num_node_features, hc_graph, edge_dim=edge_dim, heads=heads, dropout=gnn_dropout))
        for i in range(1, num_hops):
            self.gat_layers.append(GATConv(hc_graph*heads, hc_graph, edge_dim=edge_dim, heads=heads, dropout=gnn_dropout))

        # ---------------------------
        # shared linear layers
        # ---------------------------
        # concat node0 data type tensor with GNN hidden state
        shared_input_dim = hc_graph*heads + TypeEncoder.tensor_size()
        self.shared_linear_layers = create_linear_stack(num_shared_layers, shared_input_dim, hc_linear, shared_dropout)
        ...
        self.ptr_l1_head = create_linear_stack(num_task_layers-1, hc_linear, hc_task, task_dropout)
        self.ptr_l2_head = create_linear_stack(num_task_layers-1, hc_linear, hc_task, task_dropout)
        self.ptr_l3_head = create_linear_stack(num_task_layers-1, hc_linear, hc_task, task_dropout)

        self.leaf_category_head = create_linear_stack(num_task_layers-1, hc_linear, hc_task, task_dropout)
        self.leaf_signed_head   = create_linear_stack(num_task_layers-1, hc_linear, hc_task, task_dropout)
        self.leaf_floating_head = create_linear_stack(num_task_layers-1, hc_linear, hc_task, task_dropout)
        ...
        self.leaf_bool_head     = create_linear_stack(num_task_layers-1, leafbool_in_dim, hc_task, task_dropout)
        ...
        self.leaf_size_head     = create_linear_stack(self.num_leafsize_layers-1, leafsize_in_dim, self.hc_leafsize, task_dropout)
    ...
    def forward(self, x, edge_index, batch, edge_attr=None):
        ...
        # GNN layers
        # ----------
        # NOTE: it's tempting to downselect to node0 indices here, but we would
        # not be passing ALL NODES through the network, just node 0.
